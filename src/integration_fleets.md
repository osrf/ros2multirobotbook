...
The C++ API for **Full Control** automated guided vehicle (AGV) fleets can be found in the [`rmf_fleet_adapter`](https://github.com/open-rmf/rmf_ros2/tree/main/rmf_fleet_adapter) package of the `rmf_ros2` repo. The API consists of five critical classes:
...
With the robots controlled and managed by their respective fleets, robots of different models with varying configurations (such as task capabilities, vehicle traits and communication interface) can coexist and operate simultaneously in the same environment under RMF.
...
This fleet adapter implementation uses REST API with FastAPI's framework. Apart from the integrated client, users can also interact with the manager's endpoints with FastAPI's automatic documentation. This can be assessed by launching any of the demo worlds and visiting the relevant fleet's base URL followed by `/docs`. For example, to view Office world's `tinyRobot` fleet documentation, launch Office world in Gazebo with the following command:
```
ros2 launch rmf_demos_gz office.launch.xml
```
And then visit [http://127.0.0.1:22011/docs](http://127.0.0.1:22011/docs). The port number `22011` is specific to the `tinyRobot` fleet. Each fleet uses a different port number, which can be found in the respective fleet config files.
